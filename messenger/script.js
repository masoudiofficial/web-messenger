...
document.querySelectorAll('.xhorizontalscroll').forEach((xhscrollable) => {
    let isMouseDown = false;
    let startX = 0;
    let scrollStart = 0;
    let isDragging = false;
    xhscrollable.addEventListener('mousedown', (event) => {
        startX = event.clientX;
        scrollStart = xhscrollable.scrollLeft;
        isMouseDown = true;
        isDragging = false;
    });
    xhscrollable.addEventListener('mousemove', (event) => {
        if (!isMouseDown)
            return;
        ...
        if (isDragging) {
            xhscrollable.scrollLeft = scrollStart - deltaX;
...
document.querySelectorAll('.xverticalscroll').forEach((xvscrollable) => {
    let isMouseDown = false;
    let startY = 0;
    let scrollStart = 0;
    let isDragging = false;
    xvscrollable.addEventListener('mousedown', (event) => {
        startY = event.clientY;
        scrollStart = xvscrollable.scrollTop;
        isMouseDown = true;
        isDragging = false;
    });
    xvscrollable.addEventListener('mousemove', (event) => {
        if (!isMouseDown)
            return;
        ...
        if (isDragging) {
            xvscrollable.scrollTop = scrollStart - deltaY;
            event.preventDefault();
        }
    });
    document.addEventListener('mouseup', () => {
...
async function ximagetoupload(xselectfile) {
    ...
    if (allowedTypes.includes(xfiletype) && allowedExtensions.includes(xfilename.split('.').pop())) {
        ...
        document.querySelector('.xallowedformats').innerText = xfilename + ' [ ' + xbytes.toFixed(2) + ' ' + xunits[j] + ' ]';
        ...
        xalertmessage("Invalid format !", "#ffa500");
    }
}
document.getElementById("xselectimage").addEventListener("click", function () {
    document.getElementById('ximagetoupload').click();
    document.getElementById('ximagetoupload').addEventListener("change", (event) => ximagetoupload(event.target));
});

async function xcirclegraph() {
    const formData = new FormData();
    ...
    try {
        ...
        if (responseJson && Object.keys(responseJson).length > 0) {
            ...
            var xvideoangle = responseJson.xvideoangle;
            var xfreeangle = responseJson.xfreeangle;
            ...
            xcirclegraph.style.background = 'conic-gradient(' +
                    ...
                    ')';
            var xlabels = [
                {text: '<span class="xred">▪ </span>' + 'audios : ' + responseJson.xaudiosize, angle: xaudioangle / 2},
                {text: '<span class="xgreen">▪ </span>' + 'images : ' + responseJson.ximagesize, angle: xaudioangle + ximageangle / 2},
                {text: '<span class="xblue">▪ </span>' + 'videos : ' + responseJson.xvideosize, angle: xaudioangle + ximageangle + xvideoangle / 2},
                {text: '<span class="xgray">▪ </span>' + 'free-space : ' + responseJson.xfreespace, angle: xaudioangle + ximageangle + xvideoangle + xfreeangle / 2}
            ];
            var xpreviouscoords = [];
...
["xmessage", "xreceiver"].forEach(xid => {
...
function xcharactersvalidation(xcharactersvalidation) {
    ...
    if (xcharactersvalidation === 'xmessage') {
        if (!/^[a-zA-Z0-9!;:?.,\s\p{Emoji_Presentation}_-]*$/u.test(xcharactersvalidationinput)) {
            xalertmessage('1000 allowed characters : a-z A-Z 0-9 ! ; : ? . , _ -', "#ffa500");
        }
        xcharactersvalidationinput = xcharactersvalidationinput.replace(/[^a-zA-Z0-9!;:?.,\s\p{Emoji_Presentation}_-]/gu, '');
    }
    if (xcharactersvalidation === 'xreceiver') {
        if (!/^[a-z0-9]*$/.test(xcharactersvalidationinput)) {
            xalertmessage('32 allowed characters : 0-9 a-z', "#ffa500");
        }
...
document.getElementById("xmessage").onkeyup = function () {
    document.getElementById("xlength").innerText = (1000 - this.value.length);
};
        ...
        if (xmessage !== '' || xfiles.length !== 0) {
            if (document.getElementById('xmessage').value.length <= 1000) {
                try {
                    ...
                    document.getElementById('xlength').innerText = '1000';
...
document.querySelector('.xsubmit').addEventListener("click", xsubmitmessage);

let xset_receivemessages = null;
function xreceivemessages_set() {
    document.getElementById('xmessage').value = '';
    ...
    document.getElementById('xuploadfilesbutton').style.color = '#239f40';
    document.getElementById('xuploadfiles').value = '';
    xset_receivemessages = setInterval(function () {
        xreceivemessages();
    }, 1000);
}
xreceivemessages_set();

function xreceivemessages_clear() {
    if (xset_receivemessages !== null) {
        clearInterval(xset_receivemessages);
        xset_receivemessages = null;
    }
}

async function xreceivemessages() {
    const formData = new FormData();
    ...
    try {
        ...
        const responseText = await response.text();
        if (responseText) {
            document.getElementById("xmessages").innerHTML = responseText;
            if (responseText !== 'There is no message !') {
...
function xafterrequestreceivemessages() {
    ...
    xmsgs2.forEach(xmsg2 => {
        xmsg2.addEventListener("click", function (event) {
            if (xmsg2.className === 'xbfm') {
                xreceivemessages_clear();
                xreceivemessage_set(xmsg2.getAttribute('datavalue'));
            }
...
let xset_receivemessage = null;
function xreceivemessage_set(xreceivemessage_set) {
    if (document.getElementById('xreceiver').style.display !== 'none') {
        document.getElementById('xopenoptions').style.display = 'inline-block';
        document.getElementById('xreceiver').value = xreceivemessage_set;
        ...
        document.getElementById('xuploadfilesbutton').style.color = '#239f40';
        document.getElementById('xuploadfiles').value = '';
        xset_receivemessage = setInterval(function () {
            xreceivemessage();
        }, 1000);
    }
}

function xreceivemessage_clear() {
    if (xset_receivemessage !== null) {
        clearInterval(xset_receivemessage);
        xset_receivemessage = null;
    }
}

async function xreceivemessage() {
    const formData = new FormData();
...
function xafterrequestreceivemessage() {
...
document.getElementById("xbackmsg").addEventListener("click", function () {
    document.getElementById('xactionsbar').style.display = 'none';
    document.getElementById('xdelmsgdate').value = '';
    xreceivemessage_clear();
    xreceivemessages_set();
});

async function xdelmsg() {
    const formData = new FormData();
    ...
    try {
        ...
        const responseText = await response.text();
        if (responseText && responseText.includes('Unfortunately, there is a problem !')) {
            document.getElementById("xmessages").innerHTML = responseText;
        }
    } catch (error) {
        xalertmessage("Unfortunately, there is a problem !", "#ffa500");
...
document.getElementById("xdelmsg").addEventListener("click", xdelmsg);

async function xdelmsgs() {
    const formData = new FormData();
...
document.getElementById("xdelmsgs").addEventListener("click", xdelmsgs);

async function xblockuser() {
    const formData = new FormData();
...
document.getElementById("xblockuser").addEventListener("click", xblockuser);
...
document.querySelector('.xlogout').addEventListener("click", xlogout);

document.addEventListener("contextmenu", function (e) {
    if (e.target.matches("img, video, a")) {
        e.preventDefault();
    }
});

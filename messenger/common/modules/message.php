...
if (xauthentication()) {
    ...
    class Message {
        ...
        private $xconnection = null, $key1 = "", $key2 = "", $xaccountimage = "";
        ...
        private function xdatasize($xconnection, $username) {
        ...
        private function xcontentsize($path, $ext = []) {
        ...
        private function xencryptmessages($xmessages, $key1, $key2, $creationdate) {
        ...
        private function xdecryptmessages($xmessages, $key1, $key2, $creationdate) {
        ...
        public function xsubmitmessages() {

            if ($_SESSION['user-username'][1] <= 16) {
                ...
                try {
                    ...
                    if (isset($_POST["xmessagesubmitmessage"]) && !empty($_POST['xmessagesubmitmessage']) && preg_match('/^[a-zA-Z0-9!;:?.,\s\p{Emoji_Presentation}_-]*$/u', $_POST['xmessagesubmitmessage']) && strlen($_POST['xmessagesubmitmessage']) <= 1000 && strpos($_POST['xmessagesubmitmessage'], 'RM') !== false && $_POST['xreceiversubmitmessage'] === $_SESSION['user-username'][0]) {
                        ...
                        $xselect1 = $this->xconnection->prepare("SELECT blocks FROM userstable WHERE username=? AND username2=?");
                        $xselect1->execute([$_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                        ...
                        if (!empty($xselect1_r) && strpos($xselect1_r['blocks'], $xblock) !== false) {
                            ...
                            $xupdate1 = $this->xconnection->prepare("UPDATE userstable SET blocks=? WHERE username=? AND username2=?");
                            $xupdate1->execute([str_replace('(b)' . $xblock . '(bb)', '', $xselect1_r['blocks']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                        }
                    } else {

                        $xdatetime = $this->xdatetime();
                        ...
                        #if (isset($_POST["xmessagesubmitmessage"]) && !empty($_POST['xmessagesubmitmessage']) && preg_match('/^[a-zA-Z0-9!;:?.,\s' . $xunicodeemoji . '_-]*$/u', $_POST['xmessagesubmitmessage']) && strlen($_POST['xmessagesubmitmessage']) <= 1000) {
                        if (isset($_POST["xmessagesubmitmessage"]) && !empty($_POST['xmessagesubmitmessage']) && preg_match('/^[a-zA-Z0-9!;:?.,\s\p{Emoji_Presentation}_-]*$/u', $_POST['xmessagesubmitmessage']) && strlen($_POST['xmessagesubmitmessage']) <= 1000) {
                        ...
                        if (isset($_FILES['xuploadfiles']) && isset($_FILES['xuploadfiles']['tmp_name']) && is_array($_FILES['xuploadfiles']['tmp_name']) && count(array_filter($_FILES['xuploadfiles']['tmp_name'], 'is_uploaded_file')) === count($_FILES['xuploadfiles']['tmp_name'])) {
                            if (array_sum(array_filter($_FILES['xuploadfiles']['size'])) < 104857600) {
                                if (count(array_filter($_FILES['xuploadfiles']['tmp_name'], 'is_uploaded_file')) <= 10) {

                                    foreach ($_FILES['xuploadfiles']['tmp_name'] as $xkeys => $xtmpname) {
                                        $xfilemime = mime_content_type($xtmpname);
                                        $allowedMimes = ['audio/mpeg', 'audio/ogg', 'audio/wav', 'audio/webm', 'image/gif', 'image/jpeg', 'image/png', 'image/webp', 'video/mp4', 'video/ogg', 'video/webm'];
                                        $xfileext = strtolower(pathinfo($_FILES['xuploadfiles']['name'][$xkeys], PATHINFO_EXTENSION));
                                        $allowedExts = ['gif', 'jpeg', 'jpg', 'mp3', 'mp4', 'ogg', 'ogv', 'png', 'wav', 'webm', 'webp'];
                                        if (in_array($xfilemime, $allowedMimes) && in_array($xfileext, $allowedExts)) {
                                            if ($_FILES['xuploadfiles']['size'][$xkeys] > 10485760) {
                                                echo json_encode(array("xmessage" => "Your file size is more than 10 MB !", "xtype" => "#ffa500"));
                                                $xalert++;
                                                break;
                                            }
                                        } else {
                                    ...
                                    if ($xalert === 0) {
                                        ...
                                        $xmesize = $this->xcontentsize($xmepath, ['gif', 'jpeg', 'jpg', 'mp3', 'mp4', 'ogg', 'ogv', 'png', 'wav', 'webm', 'webp']) + $this->xdatasize($xconnection = $this->xconnection, $xmepath);
                                        $xreceiversize = $this->xcontentsize($xreceiverpath, ['gif', 'jpeg', 'jpg', 'mp3', 'mp4', 'ogg', 'ogv', 'png', 'wav', 'webm', 'webp']) + $this->xdatasize($xconnection = $this->xconnection, $xreceiverpath);
                                    ...
                                    echo json_encode(array("xmessage" => "More than 10 files are not allowed !", "xtype" => "#ffa500"));
                                    $xalert++;
                                }
                            } else {
                                echo json_encode(array("xmessage" => "Your file(s) size is more than 100 MB !", "xtype" => "#ffa500"));
                                $xalert++;
                            }
                        }
                        ...
                        if ($xalert === 0) {
                            ...
                            $xselect2 = $this->xconnection->prepare("SELECT creationdate, messages, blocks FROM userstable WHERE username=? AND username2=?");
                            $xselect2->execute([$_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                            $xselect2_r = $xselect2->fetch(PDO::FETCH_ASSOC);

                            if (!empty($xselect2_r)) {#😂
                                $xmemessages = $this->xdecryptmessages($xmessages = $xselect2_r['messages'], $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']);

                                if ($xmemessages !== false) {

                                    $xselect3 = $this->xconnection->prepare("SELECT creationdate, messages, blocks FROM userstable WHERE username=?");
                                    $xselect3->execute([$_POST['xreceiversubmitmessage']]);
                                    $xselect3_r = $xselect3->fetch(PDO::FETCH_ASSOC);

                                    if (!empty($xselect3_r)) {

                                        $xreceivermessages = $this->xdecryptmessages($xmessages = $xselect3_r['messages'], $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect3_r['creationdate']);
                                    } else {
                                        $xreceivermessages = $this->xdecryptmessages($xmessages = '', $key1 = $this->key1, $key2 = $this->key2, $creationdate = $this->xdatetime());
                                    }
                                            ...
                                            if ($_POST['xreceiversubmitmessage'] !== $_SESSION['user-username'][0]) {
                                                if (!empty($xselect3_r)) {
                                                    ...
                                                    if (strpos($xselect3_r['blocks'], '(b)' . $_SESSION['user-username'][0] . '(bb)') === false) {
                                                        if (strpos($xselect2_r['blocks'], '(b)' . $_POST['xreceiversubmitmessage'] . '(bb)') === false) {
                                                            ...
                                                            if ($xmemessages !== '') {
                                                                if (strpos($xmemessages, '(u)' . $_POST['xreceiversubmitmessage'] . '(uu)') === false) {
                                                                    $xupdate2 = $this->xconnection->prepare("UPDATE userstable SET messages=CONCAT(messages, ?) WHERE username=? AND username2=?");
                                                                    $xupdate2->execute([$this->xencryptmessages($xmessages = '(p)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)' . $xmefilespath . $xmessagesubmitmessage . '(mm)(d)' . $xdatetime . '(dd)(u)' . $_POST['xreceiversubmitmessage'] . '(uu)(cc)(pp)', $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                                } else {
                                                                    $xupdate3 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=? AND username2=?");
                                                                    $xupdate3->execute([$this->xencryptmessages($xmessages = str_replace('(u)' . $_POST['xreceiversubmitmessage'] . '(uu)(cc)(pp)', '(u)' . $_POST['xreceiversubmitmessage'] . '(uu)(cc)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)' . $xmefilespath . $xmessagesubmitmessage . '(mm)(d)' . $xdatetime . '(dd)(u)' . $_POST['xreceiversubmitmessage'] . '(uu)(cc)(pp)', $xmemessages), $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                                }
                                                            } else {
                                                                $xupdate4 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=? AND username2=?");
                                                                $xupdate4->execute([$this->xencryptmessages($xmessages = '(p)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)' . $xmefilespath . $xmessagesubmitmessage . '(mm)(d)' . $xdatetime . '(dd)(u)' . $_POST['xreceiversubmitmessage'] . '(uu)(cc)(pp)', $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                            }
                                                        } else {

                                                            if ($xmemessages !== '') {
                                                                if (strpos($xmemessages, '(u)' . $_SESSION['user-username'][0] . '(uu)') === false) {
                                                                    $xupdate5 = $this->xconnection->prepare("UPDATE userstable SET messages=CONCAT(messages, ?) WHERE username=? AND username2=?");
                                                                    $xupdate5->execute([$this->xencryptmessages($xmessages = '(p)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)You have blocked ' . $_POST['xreceiversubmitmessage'] . ' ! To unblock, send the word RM' . $_POST['xreceiversubmitmessage'] . ' in this conversation !(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                                } else {
                                                                    $xupdate6 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=? AND username2=?");
                                                                    $xupdate6->execute([$this->xencryptmessages($xmessages = str_replace('(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', '(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)You have blocked ' . $_POST['xreceiversubmitmessage'] . ' ! To unblock, send the word RM' . $_POST['xreceiversubmitmessage'] . ' in this conversation !(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $xmemessages), $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                                }
                                                            } else {
                                                                $xupdate7 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=? AND username2=?");
                                                                $xupdate7->execute([$this->xencryptmessages($xmessages = '(p)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)You have blocked ' . $_POST['xreceiversubmitmessage'] . ' ! To unblock, send the word RM' . $_POST['xreceiversubmitmessage'] . ' in this conversation !(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                            }
                                                        }
                                                    } else {
                                                        ...
                                                        if ($xmemessages !== '') {
                                                            if (strpos($xmemessages, '(u)' . $_SESSION['user-username'][0] . '(uu)') === false) {
                                                                $xupdate8 = $this->xconnection->prepare("UPDATE userstable SET messages=CONCAT(messages, ?) WHERE username=? AND username2=?");
                                                                $xupdate8->execute([$this->xencryptmessages($xmessages = '(p)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)' . $_POST['xreceiversubmitmessage'] . ' blocked you !(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                            } else {
                                                                $xupdate9 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=? AND username2=?");
                                                                $xupdate9->execute([$this->xencryptmessages($xmessages = str_replace('(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', '(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)' . $_POST['xreceiversubmitmessage'] . ' blocked you !(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $xmemessages), $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                            }
                                                        } else {
                                                            $xupdate10 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=? AND username2=?");
                                                            $xupdate10->execute([$this->xencryptmessages($xmessages = '(p)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)' . $_POST['xreceiversubmitmessage'] . ' blocked you !(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                        }
                                                    }
                                                } else {
                                                    ...
                                                    if ($xmemessages !== '') {
                                                        if (strpos($xmemessages, '(u)' . $_SESSION['user-username'][0] . '(uu)') === false) {
                                                            $xupdate11 = $this->xconnection->prepare("UPDATE userstable SET messages=CONCAT(messages, ?) WHERE username=? AND username2=?");
                                                            $xupdate11->execute([$this->xencryptmessages($xmessages = '(p)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)User with username ' . $_POST['xreceiversubmitmessage'] . ' not found !(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                        } else {
                                                            $xupdate12 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=? AND username2=?");
                                                            $xupdate12->execute([$this->xencryptmessages($xmessages = str_replace('(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', '(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)User with username ' . $_POST['xreceiversubmitmessage'] . ' not found !(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $xmemessages), $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                        }
                                                    } else {
                                                        $xupdate13 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=? AND username2=?");
                                                        $xupdate13->execute([$this->xencryptmessages($xmessages = '(p)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)User with username ' . $_POST['xreceiversubmitmessage'] . ' not found !(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                    }
                                                }
                                            }
                                                    ...
                                                    if (strpos($xselect2_r['blocks'], '(b)' . $_POST['xreceiversubmitmessage'] . '(bb)') === false) {
                                                        ...
                                                        if ($xreceivermessages !== '') {
                                                            if (strpos($xreceivermessages, '(u)' . $_SESSION['user-username'][0] . '(uu)') === false) {
                                                                $xupdate14 = $this->xconnection->prepare("UPDATE userstable SET messages=CONCAT(messages, ?) WHERE username=?");
                                                                $xupdate14->execute([$this->xencryptmessages($xmessages = '(p)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)' . $xreceiverfilespath . $xmessagesubmitmessage . '(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect3_r['creationdate']), $_POST['xreceiversubmitmessage']]);
                                                            } else {
                                                                $xupdate15 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=?");
                                                                $xupdate15->execute([$this->xencryptmessages($xmessages = str_replace('(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', '(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)' . $xreceiverfilespath . $xmessagesubmitmessage . '(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $xreceivermessages), $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect3_r['creationdate']), $_POST['xreceiversubmitmessage']]);
                                                            }
                                                        } else {
                                                            $xupdate16 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=?");
                                                            $xupdate16->execute([$this->xencryptmessages($xmessages = '(p)(c)(w)' . $_SESSION['user-username'][0] . '(ww)(m)' . $xreceiverfilespath . $xmessagesubmitmessage . '(mm)(d)' . $xdatetime . '(dd)(u)' . $_SESSION['user-username'][0] . '(uu)(cc)(pp)', $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect3_r['creationdate']), $_POST['xreceiversubmitmessage']]);
                                                        }
                                                    }
                                                }
                                            }
        ...
        public function xreceivemessages() {

            try {

                $xdatetime = $this->xdatetime();

                $xselect1 = $this->xconnection->prepare("SELECT creationdate, messages FROM userstable WHERE username=? AND username2=?");
                $xselect1->execute([$_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                $xselect1_r = $xselect1->fetch(PDO::FETCH_ASSOC);

                if (!empty($xselect1_r)) {#😂
                    $xmemessages = $this->xdecryptmessages($xmessages = $xselect1_r['messages'], $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect1_r['creationdate']);

                    if ($xmemessages !== false) {

                        if ($xselect1_r['messages'] !== '') {

                            if (strpos($xmemessages, '(p)(pp)') !== false) {
                                $xupdate1 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=? AND username2=?");
                                $xupdate1->execute([$this->xencryptmessages($xmessages = str_replace('(p)(pp)', '', $xmemessages), $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect1_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                            } else {
                                ...
                                $xselect2 = $this->xconnection->prepare("SELECT creationdate, messages, status FROM userstable WHERE username=?");
                                ...
                                for ($i = 0; $i < count($xparent); $i++) {
                                    ...
                                    $xselect2->execute([$xusername]);
                                    $xselect2_r = $xselect2->fetch(PDO::FETCH_ASSOC);

                                    if (!empty($xselect2_r)) {
                                        $xreceivermessages = $this->xdecryptmessages($xmessages = $xselect2_r['messages'], $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']);
                                        ...
                                        if ($xreceivermessages !== false) {
                                            ...
                                            if ((abs(strtotime($xdatetime)) - abs(strtotime($xselect2_r['status']))) < 2) {
                                                $xstatus = 'online';
                                            }
                            ...
                            $xupdate2 = $this->xconnection->prepare("UPDATE userstable SET status=? WHERE username=? AND username2=?");
                            $xupdate2->execute([$xdatetime, $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                        } else {
        ...
        public function xreceivemessage() {

            try {

                $xdatetime = $this->xdatetime();

                $xselect1 = $this->xconnection->prepare("SELECT creationdate, messages, status FROM userstable WHERE username=? AND username2=?");
                $xselect1->execute([$_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                $xselect1_r = $xselect1->fetch(PDO::FETCH_ASSOC);
                ...
                if (!empty($xselect1_r)) {#😂
                    ...
                    if ($xmemessages !== false) {
                        ...
                        if ($xselect1_r['messages'] !== '') {
                            ...
                            $xmestatus = 'xoff';
                            if ((abs(strtotime($xdatetime)) - abs(strtotime($xselect1_r['status']))) < 2) {
                                $xmestatus = 'xon';
                            }

                            $xselect2 = $this->xconnection->prepare("SELECT creationdate, messages, status FROM userstable WHERE username=?");
                            $xselect2->execute([$_POST['xuserreceivemessage']]);
                            $xselect2_r = $xselect2->fetch(PDO::FETCH_ASSOC);
                            ...
                            if (!empty($xselect2_r)) {
                                ...
                                if ($xreceivermessages !== false) {
                                    ...
                                    $xupdate2 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=?");
                                            ...
                                            for ($j = count($xchild) - 1; $j >= 0; $j--) {
                                                if (strpos($xchild[$j], '(w)' . $_POST['xuserreceivemessage'] . '(ww)') !== false) {
                                                    ...
                                                    if (strpos($xdate, '✓') === false) {
                                                        ...
                                                        $xupdate2->execute([$this->xencryptmessages($xmessages = str_replace($xoldparent, $xparent[$i], $xreceivermessages), $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $_POST['xuserreceivemessage']]);
                                                    }
                                                }
                                            }
                                    ...
                                    $xupdate3 = $this->xconnection->prepare("UPDATE userstable SET status=? WHERE username=? AND username2=?");
                                    $xupdate3->execute([$xdatetime, $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                } else {
                                    return false;
                                }
                            }
            ...
            try {
                ...
                if (!empty($xselect1_r)) {#😂
                    ...
                    if ($xmemessages !== false) {
                        ...
                        $xupdate1 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=? AND username2=?");
                        $xselect2 = $this->xconnection->prepare("SELECT creationdate, messages FROM userstable WHERE username=?");
                        $xupdate2 = $this->xconnection->prepare("UPDATE userstable SET messages=? WHERE username=?");
                            ...
                            if ((strpos($xparent[$i], '(w)' . $_SESSION['user-username'][0] . '(ww)') !== false) && (strpos($xparent[$i], $_POST['xdelmsgdate']) !== false)) {
                                ...
                                for ($j = $xcountchild - 1; $j >= 0; $j--) {
                                    if ((strpos($xchild[$j], '(w)' . $_SESSION['user-username'][0] . '(ww)') !== false) && (strpos($xchild[$j], $_POST['xdelmsgdate']) !== false)) {
                                        ...
                                        $xupdate1->execute([$this->xencryptmessages($xmessages = str_replace($xfind, '', $xmemessages), $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect1_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
                                                        ...
                                                        if ((strpos($xparent[$i], '(w)' . $_SESSION['user-username'][0] . '(ww)') !== false) && (strpos($xparent[$i], $_POST['xdelmsgdate']) !== false)) {
                                                            ...
                                                            for ($j = $xcountchild - 1; $j >= 0; $j--) {
                                                                if ((strpos($xchild[$j], '(w)' . $_SESSION['user-username'][0] . '(ww)') !== false) && (strpos($xchild[$j], $_POST['xdelmsgdate']) !== false)) {
                                                                    ...
                                                                    $xupdate2->execute([$this->xencryptmessages($xmessages = str_replace($xfind, '', $xreceivermessages), $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect2_r['creationdate']), $xreceiverusername]);
                    ...
                    if ($xmemessages !== false) {
                        ...
                        for ($i = count($xparent) - 1; $i >= 0; $i--) {
                            ...
                            if (strpos($xparent[$i], '(u)' . $_POST['xdelmsgsusername'] . '(uu)') !== false) {
                                ...
                                $xupdate1->execute([$this->xencryptmessages($xmessages = str_replace('(p)' . $xparent[$i], '', $xmemessages), $key1 = $this->key1, $key2 = $this->key2, $creationdate = $xselect1_r['creationdate']), $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
        ...
        public function xblockuser() {

            try {

                $xupdate1 = $this->xconnection->prepare("UPDATE userstable SET blocks=CONCAT(blocks, ?) WHERE username=? AND username2=?");
                $xupdate1->execute(['(b)' . $_POST['xblockuserusername'] . '(bb)', $_SESSION['user-username'][0], $_SESSION['user-username'][5]]);
            } catch (Throwable $e) {
                echo '<div class="xproblem">Unfortunately, there is a problem !</div>';
            } finally {
                $this->xconnection = null;
        ...
        if (xservercheck() && $_SESSION['user-username'][6] === 'index.php') {
            if (isset($_POST["xreceiversubmitmessage"]) && !empty($_POST['xreceiversubmitmessage']) && preg_match('/^[a-z0-9]+$/', $_POST['xreceiversubmitmessage']) && strlen($_POST["xreceiversubmitmessage"]) <= 32) {
                if ((isset($_POST["xmessagesubmitmessage"]) && !empty($_POST['xmessagesubmitmessage']) && preg_match('/^[a-zA-Z0-9!;:?.,\s\p{Emoji_Presentation}_-]*$/u', $_POST['xmessagesubmitmessage']) && strlen($_POST['xmessagesubmitmessage']) <= 1000) || (isset($_FILES['xuploadfiles']) && isset($_FILES['xuploadfiles']['tmp_name']) && is_array($_FILES['xuploadfiles']['tmp_name']) && count(array_filter($_FILES['xuploadfiles']['tmp_name'], 'is_uploaded_file')) === count($_FILES['xuploadfiles']['tmp_name']))) {
                    ...
                    $xmessage->xsubmitmessages();
                    unset($xmessage);
                } else {
                    echo json_encode(array("xmessage" => "There is no text or file to send !", "xtype" => "#ffa500"));
                }
            } else {
